...
class FamilyModel(Base):
    ...
    resident_id = Column(UUID(as_uuid=True), ForeignKey('m_resident.resident_id'), nullable=False)

    residents = relationship('ResidentModel', back_populates='family_rel')
    movements = relationship('FamilyMovementModel', back_populates='family_rel')
...
class FamilyMovementModel(Base):
    ...
    family = relationship('FamilyModel', back_populates='movements_rel')
